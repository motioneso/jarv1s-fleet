...
Nine units, each one pull request. Units 1 through 6 must land before the next unattended
...
most lanes that hit any bump (unit 3), can burn a model call a minute (unit 4), silently wedges
its merge step (unit 5), and finishes work without closing it out on GitHub, so the morning
board understates the night (unit 6, also observed live tonight). Units 7 through 9 follow in
order; the fleet is safe to run overnight without them, just rougher around the edges.
...
- **A stillness alarm, regardless of cause.** The general disease is silence reading as
  ...
  viewer shows it in the header next to the judge alarm from unit 7. It does not park the
...
## Unit 6: done means closed out on GitHub

**The bug, observed live on 2026-08-23.** Pull request 1914 merged and the daemon marked its
lane done, but issue 1422 is still sitting open on the project board. The daemon's private
records say the night went well; the board Ben actually looks at says the work never finished.
Over a full night that gap multiplies, and an unattended run whose results still need a human
to tidy the board has not really run unattended. Ben's ruling, 2026-08-23: finishing must
close things out automatically.

**The fix.** When a lane's pull request is confirmed merged, the daemon's existing wrap-up
step (remove the worktree, close the pane) gains two more actions before the lane may be
called done: close the issue on GitHub with a short comment linking the merged pull request,
and move the issue's entry on the project board to Done. The lane record only moves to done
once both have succeeded, so the daemon's word "done" and the board's word "Done" can never
mean different things.

**Whose job: the daemon on the tick, not the lane's last act.** The build agent is long gone
by merge time - auto-merge fires when checks finish, possibly hours after the agent stopped -
so only the daemon witnesses the merge. This also keeps the standing rule in every brief that
lane agents never touch the project board: one writer, no races between an agent and the
daemon both editing board state.

**When the close call fails or GitHub is refusing to answer.** The two new actions follow
unit 2's rules: errors are captured, a rate-limit answer means back off and retry next tick,
and an empty-but-successful answer is never trusted. The lane simply stays in its merged-but-
not-closed-out state and retries - this is safe to retry forever in principle, because both
actions are idempotent. But to keep one broken call from hiding a finished night, after three
failed attempts the daemon marks the lane done anyway with a "still open on GitHub" note on
the record, and the board gains a banner listing every such issue. The rule from unit 2
applies here in its own form: the daemon's records and the GitHub board are allowed to
disagree only loudly, never silently.

**If a human already closed the issue.** The daemon checks before acting: an issue already
closed means nothing to do, one log line, and the lane proceeds to done. Same for a board
entry already in Done. Nothing re-opens, nothing double-comments.

**The other half: work left lying around when a lane does not finish.** A parked or abandoned
lane leaves its branch and open pull request behind. The daemon does not close those, on
purpose: an unmerged pull request is unfinished work, and throwing unfinished work away is a
judgment call - it sits next to "deleting branches with unmerged work" on the deputy's hard
floor, and a wrong close at 4am destroys context Ben needs at breakfast. Instead the daemon
makes the leftovers impossible to miss: the morning board gains a "left behind" section
listing every parked lane's open branch and pull request, so nothing lies around invisibly.
When Ben rules on a parked lane (resume, merge, or re-slice), the normal flow takes over from
there. The one case the daemon already handles stays as is: a pull request that someone
closed without merging parks the lane with that as the reason.

**Tests.** Dry-run cases: a merged lane emits exactly one issue-close and one board-move
action before done; a failed close keeps the lane un-done and retries; the third failure
marks done with the still-open note and the board banner; an already-closed issue and an
already-Done board entry each proceed quietly with one log line; a parked lane's pull request
is never closed; the left-behind section lists a parked lane's branch and pull request.

**Live proof for this unit.** One real lane driven through merge on the dev box with nobody
typing: the issue observed closing with the linking comment, its board entry observed moving
to Done on project 2, and the lane record reaching done only after both. Then one lane parked
by hand and its branch and pull request observed in the board's left-behind section. Recorded
on the pull request.

---

## Unit 7: trust and hygiene
...
- **Idle backoff and a finished flag.** When every record is done or parked and intake finds
  ...
  to the human, via the viewer's end-run action (unit 9) or the STOP file.
...
## Unit 8: the lane watchdog, corrected
...
- **Pane to lane mapping, defined.** The agent list maps each live agent to its pane, and the
  fleet's own agent names carry the issue number (unit 7 makes that matching exact). A pane in
...
- **Is a time ceiling still needed? Yes, one, much longer, for the opposite wedge.** The
  ...
  forty-five minute ceiling is gone; it existed only because quiet alone was the trigger. The
  three-hour figure is Ben's decision (ruling, 2026-08-23), not a placeholder - do not reopen
  it without a live incident that contradicts the reasoning above.
...
- **Layout**, per the agreed mockup: header with run clock, live indicator, lanes against the
  ...
  alarm from unit 7 and the stillness warning from unit 2; three lines per in-progress lane (identity, progress track, one plain
...
1. **Replies to parked lanes use fixed first words** ("resume", "merge", anything else flags
   the lane for his attention). No model between his words and the action. Unit 7.
2. **Checks pending too long: re-run once, then park**, so a 3am CI outage does not cost the
   whole lane. Unit 5.
3. **Defaults are five lanes and thirty spawns** - aligned up, not down. Unit 7 carries the
...
4. **The watchdog never kills on quiet alone.** It checks the process tree first and stays
   ...
   Unit 8.
5. **The three-hour backstop number is approved as written**, so it is Ben's decision, not a
   spec author's guess to be reopened later. Unit 8.
6. **Finishing must close things out on GitHub** ("we need to have something auto close prs
   once done"): the issue closes and its board entry moves to Done when the lane finishes,
   loudly flagged when GitHub will not cooperate, and leftovers from unfinished lanes are
   listed rather than deleted. Unit 6, and it joins the group gated before the next
